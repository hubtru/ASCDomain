...
from helpers.dir_augment import DirAugment
...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.dir_augment = DirAugment(
            orig_freq=self.config.orig_sample_rate,
            new_freq=self.config.sample_rate,
            augment_p=self.config.augment_p
        )
        ...
        self.classifier = common_network.feat_classifier(
            config.n_classes, config.filters, 'linear')
        ...
        # rsc drop rates
        self.drop_f = (1 - config.rsc_f_drop_factor) * 100
        self.drop_b = (1 - config.rsc_b_drop_factor) * 100
    ...
    def mel_forward(self, x):
        ...
        # during training resampling is already part of dir_augment pipeline
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        if args.augment_p > 0:
            # device impulse augmentation
            x = self.dir_augment(x, devices)
        ...
        # RSC: Representation Self-Challenging from https://github.com/jindongwang/transferlearning/blob/master/code/DeepDG/alg/algs/RSC.py
        one_hot = F.one_hot(labels, self.config.n_classes)

        # Equation (1): compute gradients with respect to representation
        gradient = autograd.grad((y_hat * one_hot).sum(), features)[0]

        # Equation (2): compute top-gradient-percentile mask
        percentiles = np.percentile(gradient.cpu(), self.drop_f, axis=1)
        percentiles = torch.Tensor(percentiles)
        percentiles = percentiles.unsqueeze(1).repeat(1, gradient.size(1))
        mask_feature = gradient.lt(percentiles.cuda()).float()

        # Equation (3): mute top-gradient-percentile activations
        features_muted = features * mask_feature

        # Equation (4): compute muted predictions
        y_hat_muted = self.classifier(features_muted)

        # Section 3.3: Batch Percentage
        logits = F.softmax(y_hat, dim=1)
        logits_muted = F.softmax(y_hat_muted, dim=1)
        changes = (logits * one_hot).sum(1) - (logits_muted * one_hot).sum(1)
        percentile = np.percentile(changes.detach().cpu(), self.drop_b)
        mask_batch = changes.lt(percentile).float().view(-1, 1)
        mask = torch.logical_or(mask_feature, mask_batch).float()

        # Equations (3) and (4) again, this time mutting over examples
        y_hat_muted_again = self.classifier(features * mask)

        # Equation (5): update
        loss = F.cross_entropy(y_hat_muted_again, labels)
...
def train(config):
    ...
    macs_fea, params_fea = nessi.get_torch_size(pl_module.model, input_size=shape)
    macs_cls, params_cls = nessi.get_torch_size(pl_module.classifier, input_size=(1,64))
    macs = macs_fea + macs_cls
    params = params_fea + params_cls
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="RSC")
    ...
    # RSC Drop rates
    parser.add_argument('--rsc_f_drop_factor', type=float, default=0.33)
    parser.add_argument('--rsc_b_drop_factor', type=float, default=0.33)
    ...
    parser.add_argument('--augment_p', type=float, default=0.6)  # device augmentation
