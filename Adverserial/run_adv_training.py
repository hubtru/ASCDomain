...
from models import Adver_network, common_network
...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        self.discriminator = Adver_network.Discriminator(
            config.filters, config.dis_hidden, config.num_devices) # 6 devices during training
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        p = float(batch_idx + self.current_epoch * self.trainer.num_training_batches) / self.config.n_epochs / self.trainer.num_training_batches
        alpha = 2. / (1. + np.exp(-10 * p)) - 1
        
        # featurizer
        z = self.model(x)
        y_hat = self.classifier(z)
        cls_loss = F.cross_entropy(y_hat, labels, reduction="none")
        cls_loss_mean = cls_loss.mean()

        # discriminator
        disc_input = z
        disc_input = Adver_network.ReverseLayerF.apply(disc_input, alpha)
        disc_out = self.discriminator(disc_input)
        disc_loss = F.cross_entropy(disc_out, devices, reduction='none')
        disc_loss_mean = disc_loss.mean()

        # add feature and discriminator loss
        loss = cls_loss_mean + disc_loss_mean

        self.log("lr", self.trainer.optimizers[0].param_groups[0]['lr'])
        self.log("epoch", self.current_epoch)
        self.log("train/loss", loss.detach().cpu())
        self.log("train/cls_loss", cls_loss_mean.detach().cpu())
        self.log("train/disc_loss", disc_loss_mean.detach().cpu())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="Adverserial")
    ...
    # discriminator
    parser.add_argument('--dis_hidden', type=int, default=128) # hidden dimension of discriminator
    parser.add_argument('--num_devices', type=int, default=6) # number of devices during training
