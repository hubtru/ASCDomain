...
def calc_domain_weights(split=5):
    subset_fname = f"split{split}.csv"
    subset_split_file = os.path.join(dataset_config['split_path'], subset_fname)
    df = pd.read_csv(subset_split_file, delimiter='\t')
    dfa = df[df['filename'].str.contains('a.wav')]
    dfb = df[df['filename'].str.contains('b.wav')]
    dfc = df[df['filename'].str.contains('c.wav')]
    dfs1 = df[df['filename'].str.contains('s1.wav')]
    dfs2 = df[df['filename'].str.contains('s2.wav')]
    dfs3 = df[df['filename'].str.contains('s3.wav')]
    n, a, b, c, s1, s2, s3 = len(df), len(dfa), len(dfb), len(dfc), len(dfs1), len(dfs2), len(dfs3)
    weights = [n/a, n/b, n/c, n/s1, n/s2, n/s3]
