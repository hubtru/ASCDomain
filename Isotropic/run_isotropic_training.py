...
from models.convmixer_noact import get_model_noact
from models.convmixer_mix import get_model_mix
...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        # the convmixer model
        get_model_func = None
        if config.model == 'noact':
            get_model_func = get_model_noact
        elif config.model == 'mix':
            get_model_func = get_model_mix
        else:
            get_model_func = get_model

        self.model = get_model_func(
    ...
    def mel_forward(self, x):
        ...
        x = self.resample(x) # only use this for evaluation for correct number of macs else comment this out
        x = self.mel(x)
        if self.training:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment_name', type=str, default="ConvMixer")
