...
from datasets.dcase24_dir_logits import get_training_set, get_test_set, get_base_set
...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        # the mobilenet model
        self.model = get_mobilenet(width_mult=config.width, pretrained_name=config.pretrained_name, num_classes=config.n_classes)
    ...
    def predict_step(self, eval_batch, batch_idx, dataloader_idx=0):
        x, file, label, device, city = eval_batch
...
def train(config):
    ...
    early_stopping = pl.callbacks.EarlyStopping(monitor='val/loss', mode='min', patience=config.patience)
    model_checkpoint = pl.callbacks.ModelCheckpoint(monitor='val/loss', save_last=True, save_top_k=1, mode='min', filename='best')
...
def create_logits(config):
    ...
    ckpt_file = os.path.join(ckpt_dir, "best.ckpt")
    ...
    base_dl = DataLoader(dataset=get_base_set(cache_path=config.cpath,resample_rate=config.sample_rate),
                         worker_init_fn=worker_init_fn,
                         num_workers=config.num_workers,
                         batch_size=config.batch_size)
    
    predictions = trainer.predict(pl_module, dataloaders=base_dl)
    all_predictions = torch.cat([torch.as_tensor(p) for p in predictions], 0)

    # logits and info
    torch.save(all_predictions, os.path.join(out_dir, f'{config.experiment_name}_{config.subset}.pt'))
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--sample_rate', type=int, default=32000)
    # parser.add_argument('--window_length', type=int, default=800)  # in samples (corresponds to 96 ms)
    # parser.add_argument('--hop_length', type=int, default=320)  # in samples (corresponds to ~16 ms)
    # parser.add_argument('--n_fft', type=int, default=1024)  # length (points) of fft, e.g. 4096 point FFT
