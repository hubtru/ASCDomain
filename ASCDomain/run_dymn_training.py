...
class PLModule(pl.LightningModule):
    def __init__(self, config):
        ...
        # Dynamic MobileNet
        self.model = get_dymn(width_mult=config.width, pretrained_name=config.pretrained_name,
...
def train(config):
    ...
    early_stopping = pl.callbacks.EarlyStopping(monitor='val/loss', mode='min', patience=10)
    trainer = pl.Trainer(max_epochs=config.n_epochs,
                         logger=wandb_logger,
                         accelerator='gpu',
                         devices=1,
                         precision=config.precision,
                         callbacks=[pl.callbacks.ModelCheckpoint(save_last=True), early_stopping])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_name', type=str, default='dynmn10_as')
